...
| `profile/README.md` | The organisation page. Everything between the `BEGIN GENERATED` and `END GENERATED` markers is rendered from `blueprints.yaml`, so edit the index and not the page. One table per category, with a column linking the repository of each platform. |
